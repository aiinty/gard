...
final class SpeakerCaster {
	...
	public function part_speak( $html, $post_id ) {
        ...
        if ( SettingsFields::get_voice_type( $lang_name ) !== 'standard' ) {
	...
	public function get_player( $id = 0 ) {
        ...
        if ( isset( $_GET['speaker-lite-ssml'] )  ) { return false; }

        /** Don't show player if in Speech Template Editor. */
        if ( isset( $_GET['speaker_lite_speech_template'] ) && '1' === $_GET['speaker_lite_speech_template'] ) { return false; }
			...
			<?php if ( in_array( Settings::get_instance()->options['link'], [ 'frontend', 'backend-and-frontend' ] ) ) : ?>
				<p class="mdp-speaker-lite-download-box">
					...
					<a href="<?php echo esc_url( $audio_url ); ?>"
					   download="" title="<?php echo esc_attr__( 'Download: ', 'speaker-lite' ) . htmlentities( get_the_title( $id ) ); ?>"><?php echo htmlentities( get_the_title( $id ) ); ?></a>
				</p>
			<?php endif; ?>
		</div>
		<?php
	...
	public function the_player() {
        ...
        <div class="mdp-speaker-lite-audio-info">
			<?php if ( in_array( Settings::get_instance()->options['link'], [ 'backend', 'backend-and-frontend' ] ) ) : ?>
                <span class="dashicons dashicons-download" title="<?php esc_html_e( 'Download audio', 'speaker-lite' ); ?>"></span>
                <a href="<?php echo esc_url( $audio_url ); ?>" download=""><?php esc_html_e( 'Download audio', 'speaker-lite' ); ?></a><br>
			<?php endif; ?>
            <span class="dashicons dashicons-clock" title="<?php esc_html__( 'Date of creation', 'speaker-lite' ) ?>"></span>
